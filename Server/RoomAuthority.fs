namespace SvgWorkspacePublicClean.Server

open System
open System.Collections.Concurrent
open FS.GG.Game.Core
open SvgWorkspacePublicClean.Domain
open SvgWorkspacePublicClean.ArenaContent
...
module RoomAuthority =
    ...
    let mutable private state = SvgWorkspacePublicClean.ArenaRules.createWith definition
    ...
    let private completeSnapshotLocked () =
        let content = SvgWorkspacePublicClean.ArenaRules.currentContent state
    ...
    let resetForTests () : unit =
        lock gate (fun () ->
            ...
            state <- SvgWorkspacePublicClean.ArenaRules.createWith definition
            sessions.Clear()
            lastSequence.Clear()
            pending.Clear()
    ...
    let private retirePlayerLocked (playerId: string) : unit =
        state <- SvgWorkspacePublicClean.ArenaRules.leave playerId state
    ...
    let private joinLocked (playerId: string) : Cell option =
        ...
        | None ->
            ...
            |> Option.map (fun spawn ->
                state <- SvgWorkspacePublicClean.ArenaRules.join playerId spawn state
    ...
    let arenaContent () = lock gate (fun () -> SvgWorkspacePublicClean.ArenaRules.currentContent state)
        ...
        lock gate (fun () ->
            ...
            for input in frontier do
                let intent =
                    ...
                    | "restart" -> SvgWorkspacePublicClean.ArenaRules.Intent.Restart
                    | "interact" -> SvgWorkspacePublicClean.ArenaRules.Intent.Interact
                    | "legacy-move" -> SvgWorkspacePublicClean.ArenaRules.Intent.LegacyMove { Col = input.TargetCol; Row = input.TargetRow }
                    | _ -> SvgWorkspacePublicClean.ArenaRules.Intent.Move { Col = input.TargetCol; Row = input.TargetRow }
                state <- SvgWorkspacePublicClean.ArenaRules.applyIntent input.PlayerId intent state
#if SVG_NETWORK_CANDIDATE
                NetworkAuthority.recordIntent input.PlayerId intent state
#endif
            state <- SvgWorkspacePublicClean.ArenaRules.advance state
